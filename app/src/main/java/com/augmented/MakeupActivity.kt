...
class MakeupActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        Toast.makeText(applicationContext, "Click on the buttons below to select awesome shades", LENGTH_LONG).show()
        ...
        Texture.builder()
                .setSource(this, R.drawable.lipshade)
                .build()
        ...
        btnTeaPink.setOnClickListener {
            startActivity(Intent(this, MakeupActivity3::class.java))
        }
    ...
    private fun checkIsSupportedDeviceOrFinish(): Boolean {
        if (ArCoreApk.getInstance().checkAvailability(this) == ArCoreApk.Availability.UNSUPPORTED_DEVICE_NOT_CAPABLE) {
            Toast.makeText(this, "Augmented Faces requires ARCore", LENGTH_LONG).show()
            finish()
            return false
        }
        ...
        openGlVersionString?.let { s ->
            if (java.lang.Double.parseDouble(openGlVersionString) < MIN_OPENGL_VERSION) {
                Toast.makeText(this, "Sceneform requires OpenGL ES 3.0 or later", LENGTH_LONG)
                        .show()
                finish()
                return false
            }
